# # autogenerated
self: super:
let
  common = {
    browserName = "firefox";
    pname = "firefox-bin";
    desktopName = "Firefox";
  };
in {
  firefox-stable = let
    version = "88.0";
    sources = {
      url =
        "https://download.cdn.mozilla.net/pub/firefox/releases/88.0/linux-x86_64/en-US/firefox-88.0.tar.bz2";
      sha512 =
        "c175be02ddb8881a6cdb06d50c63776f77e4541f8df82d4de62c870c255fe5fb30c8e34831b2573c7b8bd9ef0ffe533245fdc4ef1a083acede5313ae08e31d81";
    };
  in super.wrapFirefox ((self.firefox-bin-unwrapped.override {
    generated = { inherit version sources; };
  }).overrideAttrs (old: { src = super.fetchurl sources; })) common;
  firefox-esr = let
    version = "78.10.0esr";
    sources = {
      url =
        "https://download.cdn.mozilla.net/pub/firefox/releases/78.10.0esr/linux-x86_64/en-US/firefox-78.10.0esr.tar.bz2";
      sha512 =
        "c175be02ddb8881a6cdb06d50c63776f77e4541f8df82d4de62c870c255fe5fb30c8e34831b2573c7b8bd9ef0ffe533245fdc4ef1a083acede5313ae08e31d81";
    };
  in super.wrapFirefox ((self.firefox-bin-unwrapped.override {
    generated = { inherit version sources; };
  }).overrideAttrs (old: { src = super.fetchurl sources; })) common;
  firefox-devel = let
    version = "89.0b4";
    sources = {
      url =
        "https://download.cdn.mozilla.net/pub/firefox/releases/89.0b4/linux-x86_64/en-US/firefox-89.0b4.tar.bz2";
      sha512 =
        "6aed3463fab1fa56fc413943e6c27b6d44815cf5f62cb39d41c88985b82274707a895914f740b551a62cb4652672c18fc30d0db6d1b235e9ab88d7327e20c58c";
    };
  in super.wrapFirefox ((self.firefox-bin-unwrapped.override {
    generated = { inherit version sources; };
  }).overrideAttrs (old: { src = super.fetchurl sources; })) common;
}
