...
LATEST_FIREFOX_NIGHTLY_VERSION=$(cat $JSON_FILE |jq -r .FIREFOX_NIGHTLY)

if [[ -z "$LATEST_FIREFOX_VERSION" || -z "$LATEST_FIREFOX_DEVEL_VERSION" || -z "$LATEST_FIREFOX_ESR_VERSION" || -z "$LATEST_FIREFOX_NIGHTLY_VERSION" ]]; then
...
LATEST_FIREFOX_NIGHTLY_SHA512SUM=$(curl -s https://download.cdn.mozilla.net/pub/firefox/nightly/latest-mozilla-central/firefox-${LATEST_FIREFOX_NIGHTLY_VERSION}.en-US.linux-x86_64.checksums | awk '$4 == "firefox-'${LATEST_FIREFOX_NIGHTLY_VERSION}'.en-US.linux-x86_64.tar.bz2" && $2 == "sha512" { print $1; }')

if [[ -z "$LATEST_FIREFOX_SHA512SUM" || -z "$LATEST_FIREFOX_DEVEL_SHA512SUM" || -z "$LATEST_FIREFOX_ESR_SHA512SUM" || -z "$LATEST_FIREFOX_NIGHTLY_SHA512SUM" ]]; then
...
LATEST_FIREFOX_NIGHTLY_TARBALL="https://download.cdn.mozilla.net/pub/firefox/nightly/latest-mozilla-central/firefox-${LATEST_FIREFOX_NIGHTLY_VERSION}.en-US.linux-x86_64.tar.bz2"
...
in {
  ...
  common;
  firefox-nightly = let
    version = "$LATEST_FIREFOX_NIGHTLY_VERSION";
    sources = {
      url = "$LATEST_FIREFOX_NIGHTLY_TARBALL";
      sha512 = "$LATEST_FIREFOX_NIGHTLY_SHA512SUM";
    };
  in super.wrapFirefox ((self.firefox-bin-unwrapped.override {
    generated = {
      inherit version sources;
